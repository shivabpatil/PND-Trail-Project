angular.module('pndApp').controller('pndCustomerCtrl', function($scope,$filter,$window,$rootScope,$http,getcustomerService){
	...
	$scope.selectServiceCenter =function (area) {
		...
		$http.get("https://pndservices.herokuapp.com/api2/servicecenters?_areaId="+ area._id).success(function(res){
			$scope.serviceCenters = res;
			//console.log(res);
			//console.log($scope.serviceCenters);
		});

		$http.get("https://pndservices.herokuapp.com/api7/slots?_areaId=" + area._id ).success(function(res){
			$scope.slots = res;
		    // console.log(res);
	...
	$scope.create = function (customer,bike,schedule,serviceCenterDetails,slotDetails,area) {
		...
		$http.get("https://pndservices.herokuapp.com/api8/bookedSlots?_slotId=" + bookedSlot._slotId + "&booking_date=" + bookedSlot.booking_date ).success(function(res){
			...
			if ($scope.bookedSlots.length < slotDetails.slot_capacity){
				...
				$http.post("https://pndservices.herokuapp.com/api8/bookedSlots",bookedSlot).success(function(res){
				
					console.log(res);
					// console.log($scope.serviceCenters);
				});
				
				$http.post("https://pndservices.herokuapp.com/api3/customers",customer).success(function(res){			
						bike._customerId = res._id;
					});

				$http.post("https://pndservices.herokuapp.com/api4/bikes",bike).success(function(res){
					});

				$http.post("https://pndservices.herokuapp.com/api6/schedules",schedule).success(function(res){
						scheduleId = res._Id;
						console.log(res);
		...
		// $http.post("https://pndservices.herokuapp.com/api8/bookedSlots",).success(function(res){
		// 	$scope.serviceCenters = res;
		// 	console.log(res);
		// 	console.log($scope.serviceCenters);
		// });
